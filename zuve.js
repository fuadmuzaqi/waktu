...
(function () {
  ...
  const API_URL = 'https://fuadtm.fuad-alhabibi.workers.dev/'; // ganti sesuai worker-mu
  ...
  async function syncFromAPI(el) {
    try {
      ...
      const t2 = (typeof performance !== 'undefined' && performance.now) ? performance.now() : 0;
      ...
      baseMono = (typeof performance !== 'undefined' && performance.now) ? performance.now() : 0;
      return true;
    } catch (e) {
  ...
  function render(el) {
    if (stopped) return;
    const nowMono = (typeof performance !== 'undefined' && performance.now) ? performance.now() : 0;
    const elapsed = nowMono - baseMono;       // durasi lokal monotonic
